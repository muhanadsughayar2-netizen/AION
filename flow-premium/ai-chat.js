...
async function loadAdditionalImagesFromIndexedDB() {
  try {
    const db = await openSnapDB();
    const tx = db.transaction(SNAPTOAI_STORE_NAME, 'readwrite');
    const store = tx.objectStore(SNAPTOAI_STORE_NAME);
    const request = store.get('additionalSnaps');
    const result = await new Promise((resolve, reject) => {
      request.onsuccess = () => resolve(request.result || []);
      request.onerror = () => reject(request.error);
    });
    // Clear after loading
    store.delete('additionalSnaps');
    await new Promise((resolve) => { tx.oncomplete = resolve; });
    db.close();
    return result;
  } catch (e) {
    console.error('[SnapToAI] IndexedDB additional load failed:', e);
    return [];
  }
}

// Listen for new images added to chat
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'addImagesToChat') {
    console.log('[SnapToAI] Received addImagesToChat message');
    handleAddImagesToChat();
    sendResponse({ success: true });
  }
  return true;
});

async function handleAddImagesToChat() {
  const newImages = await loadAdditionalImagesFromIndexedDB();
  if (newImages.length === 0) return;
  
  console.log('[SnapToAI] Adding', newImages.length, 'new images to chat');
  
  // Add to current images array
  if (typeof currentImages !== 'undefined') {
  ...
  if (imageGrid) {
    newImages.forEach((imgSrc, idx) => {
      const imgContainer = document.createElement('div');
      imgContainer.className = 'image-item';
      imgContainer.innerHTML = `<img src="${imgSrc}" alt="Added image">`;
      imageGrid.appendChild(imgContainer);
    });
  }
  
  // Show notification
  showToast(`${newImages.length} image(s) added to chat`);
}
