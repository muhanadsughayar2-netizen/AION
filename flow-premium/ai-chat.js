...
function buildNoKeyCard() {
  return `
    <div style="padding:18px;border-radius:14px;background:linear-gradient(135deg, rgba(59,123,255,0.10), rgba(138,43,226,0.05));border:1px solid rgba(59,123,255,0.22);box-shadow:0 8px 32px rgba(0,0,0,0.2);">
      ...
      <div style="display:flex;flex-direction:column;gap:7px;margin-bottom:14px;">
        <div style="display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:10px;background:rgba(59,123,255,0.06);border:1px solid rgba(59,123,255,0.12);">
          <span style="font-size:14px;">1️⃣</span>
          <span style="font-size:12px;color:rgba(255,255,255,0.85);">Go to <a href="https://aistudio.google.com/apikey" target="_blank" rel="noopener" style="color:#3B7BFF;text-decoration:none;font-weight:600;">Google AI Studio</a></span>
        </div>
        <div style="display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:10px;background:rgba(59,123,255,0.06);border:1px solid rgba(59,123,255,0.12);">
      ...
      <button class="unlock-billing-btn snaptoai-set-key-btn" style="display:block;width:100%;text-align:center;padding:11px;border-radius:10px;background:linear-gradient(135deg, #3B7BFF, #8a2be2);color:#fff;font-size:13px;font-weight:700;border:none;cursor:pointer;">Enter My Key →</button>
...
function buildRateLimitCard() {
  return `
    <div style="padding:16px;border-radius:14px;background:linear-gradient(135deg, rgba(59,123,255,0.10), rgba(138,43,226,0.06));border:1px solid rgba(59,123,255,0.22);box-shadow:0 8px 32px rgba(0,0,0,0.2);">
      <div style="display:flex;align-items:center;gap:10px;margin-bottom:10px;">
        <span style="font-size:24px;">⏳</span>
...
async function showProxyKeyPrompt() {
  ...
  if (saveBtn && input) {
    const runSave = async () => {
      ...
      statusEl.style.background = 'rgba(59,123,255,0.08)';
      statusEl.style.border = '1px solid rgba(59,123,255,0.25)';
      statusEl.style.color = '#9be7ff';
      statusEl.innerHTML = '<span style="display:inline-block;width:10px;height:10px;border:2px solid #3B7BFF;border-top-color:transparent;border-radius:50%;animation:spin 0.8s linear infinite;vertical-align:middle;margin-right:8px;"></span>Checking your account for video model access…';
...
const MODEL_NAMES = {
  'vision': { name: 'Gemini 3', sub: 'Flash (Preview)', color: '#3B7BFF' },
  ...
  btns.forEach(btn => {
    btn.addEventListener('click', async () => {
      ...
      if (thread) {
        ...
        const borderColors = { 'vision': '#3B7BFF', 'image': '#ff6bed', 'music': '#00ff88', 'video': '#ffa500' };
        notice.style.borderLeftColor = borderColors[mode] || '#3B7BFF';
...
function buildVeoRerenderPanel(ctx) {
  ...
  const rows = clipResults.map((r, idx) => {
    ...
    const fixStitchBtn = prevHasUrl
      ? `<button class="veo-fix-stitch-btn" data-clip-idx="${idx}" title="Re-render this clip starting from the LAST FRAME of clip ${idx} so the join looks seamless." style="padding:4px 10px;border-radius:6px;border:1px solid rgba(59,123,255,0.4);background:rgba(59,123,255,0.1);color:#3B7BFF;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;">✂ Fix stitch (${idx}→${idx + 1})</button>`
      : '';
    ...
    if (idx > 0 && (failures[idx - 1] || userRefs[idx - 1])) {
      ...
      if (hasUserRef) {
        chainWarning = `<div style="margin-top:6px;padding:8px 10px;background:rgba(59,123,255,0.08);border:1px solid rgba(59,123,255,0.3);border-radius:8px;font-size:11px;color:#7fe7ff;line-height:1.5;display:flex;justify-content:space-between;align-items:center;gap:10px;flex-wrap:wrap;">
          <span>✓ Custom reference image set for the chain into clip ${idx + 1}. Click <b>Fix stitch</b> above to apply it.</span>
          <button class="veo-clear-userref-btn" data-prev-idx="${idx - 1}" style="padding:3px 8px;border-radius:5px;border:1px solid rgba(59,123,255,0.4);background:transparent;color:#7fe7ff;font-size:10px;font-weight:600;cursor:pointer;">Clear</button>
        </div>`;
      } else if (failReason) {
  ...
  return `<div style="margin-top:12px;background:rgba(255,165,0,0.04);border:1px solid rgba(255,165,0,0.2);border-radius:10px;padding:12px;">
    ...
    <div style="font-size:11px;color:#8899aa;margin-bottom:10px;line-height:1.5;">
      <span style="color:#3B7BFF;font-weight:600;">✂ Fix stitch</span> = same prompt, but the clip starts from the previous clip's last frame so the cut is invisible. Use this when the join looks jarring.
    </div>
    ${rows}
  </div>`;
}
...
function buildVeoSummaryCard(ctx, billingAbortAt, hasSuccess, successCount) {
  ...
  const rows = clipResults.map((r, idx) => {
    ...
    const retryBtn = canRetry
      ? `<button class="veo-retry-btn" data-clip-idx="${idx}" style="background:rgba(59,123,255,0.15);border:1px solid rgba(59,123,255,0.4);color:#3B7BFF;padding:4px 10px;border-radius:6px;font-size:11px;font-weight:600;cursor:pointer;white-space:nowrap;">🔄 Retry</button>`
  ...
  if (hasSuccess) {
    return `<div style="margin-top:14px;background:rgba(59,123,255,0.06);border:1px solid rgba(59,123,255,0.2);border-radius:10px;padding:12px;color:#fff;font-size:12px;">
      <div style="font-weight:600;margin-bottom:8px;color:#3B7BFF;">${successCount} of ${clipCount} clips succeeded · retry the rest below</div>
    ...
    <div style="background:linear-gradient(135deg, rgba(0,255,136,0.06), rgba(0,200,100,0.03));border:1px solid rgba(0,255,136,0.12);border-radius:16px;padding:20px;backdrop-filter:blur(10px);">
      ...
      <div style="background:linear-gradient(135deg, rgba(255,170,0,0.08), rgba(255,100,0,0.04));border:1px solid rgba(255,170,0,0.2);border-radius:12px;padding:14px;margin:10px 0;">
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:10px;">
          ...
          <button class="img2music-chill" style="padding:6px 12px;border-radius:10px;border:1px solid rgba(59,123,255,0.3);background:rgba(59,123,255,0.08);color:#3B7BFF;font-size:11px;cursor:pointer;">😌 Chill</button>
  ...
  if (isDirect) {
    ...
    previewContainer.innerHTML = `
      <div style="padding: 20px; text-align: center; color: #8899aa;">
        ...
        <div style="font-size: 14px; font-weight: 600; color: #3B7BFF;">Analyze Images with AI</div>
        <div style="font-size: 11px; color: #889999; margin-top: 8px; line-height: 1.5;">
...
function setupImagePanelClicks() {
  ...
  previewContainer.addEventListener('click', (e) => {
    ...
    inner.style.cssText = 'max-width:220px;border-radius:12px;overflow:hidden;border:1px solid rgba(59,123,255,0.3);box-shadow:0 4px 16px rgba(0,0,0,0.3);position:relative;';
    ...
    img.style.outline = '2px solid var(--st-accent, #3B7BFF)';
