...
A design-system pass that unifies tokens, typography, motion, spacing, focus, and brand color across every extension surface plus the server-rendered admin dashboard.
- **Token migration.** Every exact-match color literal across `flow-premium/popup.css`, `popup.html`, `options.css`, `options.html`, `welcome.html`, `annotate.css`, `annotate.html`, `magic-card.html`, `tour.css`, `sidebar.html`, `ai-chat.html`, and `templates/privacy-policy.html` now resolves through `--st-bg-app` / `--st-bg-surface` / `--st-bg-elevated` / `--st-accent` / `--st-accent-2`. Future palette tweaks in `flow-premium/theme.css` ripple into every surface in one place instead of being silently shadowed.
- **Theme-invariant tokens added.** `flow-premium/theme.css` now exposes `--st-font-sans`/`--st-font-mono`, a 4/8 spacing rhythm (`--st-space-1..--st-space-10`), a radius scale (`--st-radius-sm` → `--st-radius-pill`), motion (`--st-motion-fast/base/slow` + `--st-ease-standard`), and a `--st-focus-ring` reused by the global focus polish.
- **Global accessibility polish.** A `:where(button, [role="button"], a, input, select, textarea, summary, [tabindex]):focus-visible` rule in `theme.css` paints the shared focus ring on every interactive element on every page (popup, sidebar, AI chat, options, welcome, annotate, magic card, tour). Reduced-motion is now honored globally — `prefers-reduced-motion: reduce` clamps every animation/transition to `0.01ms` (not just the theme cycle button) while keeping `transitionend` listeners alive.
- **Brand-color alignment.** The server-rendered super-admin dashboard in `app.py` was using `#00d4ff` / `#00b8e6` (a near-but-not-matching cyan) — re-aligned to the extension's `#00d9ff` / `#00b8d4` so the admin surface matches the extension brand.
- **Light-mode parity.** Every surface that participates in the `:root[data-theme="light"]` overrides in `theme.css` (popup, sidebar, AI chat, options, welcome, annotate, magic card, tour) keeps full parity; the new tokens are theme-invariant, so dark mode renders byte-identical to before.
- **Out of scope.** Landing-page (purple/pink marketing) palette is intentionally not unified with the extension cyan — it is its own brand surface and was not in the pass. Existing tasks #12 (toast theming), #13 (light-mode popup polish), #14/15/16 (video studio polish), #23 (demo screenshots), #24 (copy invite), #25 (NEW badge) cover targeted refinements that remain tracked separately.
