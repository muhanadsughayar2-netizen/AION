...
- **AI Mode System:** 4 mode buttons in AI chat + dropdown in popup settings. Modes: Vision (Gemini 2.0 Flash, streaming text analysis), Image (gemini-2.0-flash-preview-image-generation, native image generation via generateContent with responseModalities: IMAGE), Music/Audio (gemini-2.5-flash-preview-tts, text-to-speech via generateContent with responseModalities: AUDIO), Video (Veo 2.0, predict endpoint — may require special API access). Stored as `geminiModel` in chrome.storage.sync. Mode buttons are pill-shaped chips above the input area. Popup AI always uses gemini-2.0-flash regardless of selected mode.
