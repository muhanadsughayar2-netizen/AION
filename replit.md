...
- **AI Mode System:** 4 mode buttons in AI chat + dropdown in popup settings. Modes: Vision (Gemini 2.0 Flash, streaming text analysis), Image (Imagen 3, image generation via predict endpoint), Music (Lyria, 30-sec clips), Video (Veo, short video clips). Each mode uses different API endpoint patterns. Stored as `geminiModel` in chrome.storage.sync. Mode buttons are pill-shaped chips above the input area. Popup AI always uses gemini-2.0-flash regardless of selected mode.
