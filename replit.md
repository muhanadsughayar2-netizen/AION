...
- **Multi-Clip Visual Chaining (v2.4.9):** When generating multi-clip stitched videos, each clip after the first uses the previous clip's last frame as its starting image (Veo image-to-video) so character/lighting/lens stay consistent across cuts. Implemented via `extractLastFrame()` (fetches video bytes → blob URL → seeks to duration−0.05s → canvas → JPEG base64) and `refreshTransitionFrame()` JIT cache. Storyboard generation now uses the user's selected per-clip duration instead of hardcoded 8s. Re-render and Retry paths refresh the previous clip's frame just-in-time so chaining stays correct after any clip is replaced. A dedicated "✂ Fix stitch" button on each clip 2..N re-renders that clip with the previous clip's freshly-extracted final frame for explicit join repair. If a Veo model variant rejects image input, automatically falls back once to text-only continuity for that attempt.
